...
export default function SiteRegistryManager({
  ...
  const activeSite = useSimulationStore((state) => state.activeSite);
  ...
  const [newSiteName, setNewSiteName] = useState("New land site");

  async function runOperation(
    key: string,
    payload: Record<string, unknown>,
    options?: { loadResult?: boolean; goDashboard?: boolean },
  ) {
    setBusyKey(key);
    setMessage("");
    ...
    try {
      ...
      if (!response.ok || !data.ok) {
        throw new Error(data.error ?? "The site operation failed.");
      }

      if (options?.loadResult && data.result) {
        replaceActiveSite(data.result.siteProfile);
      }
      ...
      setMessage(
        error instanceof Error ? error.message : "Unknown site operation error.",
      );
    } finally {
      setBusyKey(null);
    }
  ...
  return (
    <div className="space-y-6">
      ...
      {activeProject ? (
        <section className="rounded-2xl border border-slate-200 bg-white p-6">
          ...
          <p className="mt-2 text-slate-600">
            The new site starts from the current dashboard configuration, receives
            a new identity and is preserved as its own immutable site version.
          </p>
          <div className="mt-4 flex flex-col gap-3 sm:flex-row">
            ...
            >
              {busyKey === "create" ? "Creating…" : "Create land site"}
            </button>
          </div>
        </section>
      ) : null}
      ...
      {projects.map((project) => {
        const activeSites = project.sites.filter((site) => site.status === "active");
        const archivedSites = project.sites.filter(
        ...
        return (
          ...
          >
            ...
            <p className="mt-1 text-sm text-slate-500">Project ID: {project.id}</p>

            <div className="mt-5 space-y-3">
              {activeSites.map((site) => {
                const isActive = workspace.activeSiteId === site.id;
                    ...
                    <div className="flex flex-wrap items-start justify-between gap-3">
                      <div>
                        ...
                        <p className="mt-1 text-sm text-slate-500">
                          {site.siteType.replaceAll("_", " ")} · {site.dataMode}
                        </p>
                      </div>
                      ...
                      <div className="flex flex-wrap gap-2">
                        <button
                          ...
                          onClick={() =>
                            runOperation(
                              ...
                              { loadResult: true, goDashboard: true },
                            )
                          }
                        ...
                        <button
                          ...
                          onClick={() => {
                            ...
                            if (name) {
                              void runOperation(`duplicate-${site.id}`, {
                                action: "duplicate",
                                siteId: site.id,
                                name,
                              });
                        ...
                        <button
                          ...
                          disabled={busyKey !== null}
                          onClick={() => {
                            const name = window.prompt("New site name:", site.name);
                            if (name && name !== site.name) {
                              void runOperation(`rename-${site.id}`, {
                                action: "rename",
                                siteId: site.id,
                                name,
                              });
                        ...
                        <button
                          type="button"
                          disabled={busyKey !== null || activeSites.length <= 1}
                          onClick={() =>
                            runOperation(`archive-${site.id}`, {
                              action: "archive",
                              siteId: site.id,
                            })
                          }
            ...
            {archivedSites.length > 0 ? (
              <section className="mt-6">
                <h3 className="font-medium text-slate-800">Archived sites</h3>
